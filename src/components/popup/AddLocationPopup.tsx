...
import { createNewLocation } from "../../helpers/helpers";
...
const LocationRow = ({
  ...
  const [placelat] = useState(coordinate.lat);
  const [placelong] = useState(coordinate.long);
...
const LocationPopup = ({
  ...
  const [area, setArea] = useInputState();
  ...
  const handleAddition = async () => {
    ...
    try {
      if (1) {
        ...
        addToast("A new location Created", {
          autoDismiss: true,
          appearance: "success",
        });
      }
